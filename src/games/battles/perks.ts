// import { Perk } from "./perks/perkPack/iPerks";
...
class LifeExchange extends Perk {
    name = "[Life Exchange]";
    description = "\"Sacrifice 2 HP to gain 1 points immediately. If HP is less than 2, it will be set to 1 instead.\"";
    icon = "https://i.postimg.cc/KzQk4v23/Screenshot-2025-05-01-233656.png";
    ...
    effect(player1: number, player2: number, battle: any): void {
        ...
        if (isPlayer1) {
            if (battle.player1Hp > 2) {
                battle.player1Hp -= 2;
                battle.player1Points += 1;
                this.log(`<${player1Name}> sacrificed 2 HP to gain 1 points (HP: ${battle.player1Hp}, Points: ${battle.player1Points})`, false);
            } else {
                const originalHp = battle.player1Hp;
                battle.player1Hp = 1;
                battle.player1Points += 1;
                this.log(`<${player1Name}> sacrificed ${originalHp - 1} HP to gain 1 points (HP: ${battle.player1Hp}, Points: ${battle.player1Points})`, false);
            }
        } else {
            if (battle.player2Hp > 2) {
                battle.player2Hp -= 2;
                battle.player2Points += 1;
                this.log(`<${player2Name}> sacrificed 2 HP to gain 1 points (HP: ${battle.player2Hp}, Points: ${battle.player2Points})`, true);
            } else {
                const originalHp = battle.player2Hp;
                battle.player2Hp = 1;
                battle.player2Points += 1;
                this.log(`<${player2Name}> sacrificed ${originalHp - 1} HP to gain 1 points (HP: ${battle.player2Hp}, Points: ${battle.player2Points})`, true);
            }
        }
    }
}
